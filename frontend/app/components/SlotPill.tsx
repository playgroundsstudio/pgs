...
export default function SlotPill({mode, isVisible, onToggleMode, onClose}: SlotPillProps) {
  const hasClose = !!onClose

  return (
    <div
      className={cn(
        'absolute top-2 right-2 h-[32px] min-w-[50px] z-40 flex items-center rounded-full bg-pill backdrop-blur-[80px] shadow-[0_0_20px_rgba(0,0,0,0.08)] border border-border-subtle transition-all duration-300 ease-out',
        hasClose ? 'gap-4 px-4 py-2' : 'justify-center w-9 h-9',
